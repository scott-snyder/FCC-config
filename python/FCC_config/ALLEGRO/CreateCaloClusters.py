...
def CaloClusterCfg (flags,
    ...
    if doPhotonID is None:
        doPhotonID = clusterFlags.runPhotonIDTool
...
class ClusterFlags:
    def __init__ (self, opts):
        ...
        self.createClusterCellCollection = opts.createClusterCellCollections
        ...
        # only on ECAL so far)
        self.applyMVAClusterEnergyCalibration = opts.calibrateClusters
        ...
        self.runPhotonIDTool = False
        self.runPhotonIDTool = opts.runPhotonID
...
def defineCaloClusterFlags(flags, opts):
    flags.CaloSW = ClusterFlags(opts)
